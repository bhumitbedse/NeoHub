-- ─────────────────────────────────────────────────────────────────────────────
--  V1 — Initial Schema
--  Creates all core tables for NeoHub
-- ─────────────────────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS plugins (
    ...
    full_name           VARCHAR(400) UNIQUE NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_plugins_stars ON plugins(github_stars DESC);
...
CREATE OR REPLACE TRIGGER trigger_plugin_search_vector
...
CREATE INDEX IF NOT EXISTS idx_configs_public ON configs(is_public, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_configs_tags ON configs USING GIN(use_case_tags);
...
CREATE INDEX IF NOT EXISTS idx_threads_plugin ON threads(plugin_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_threads_config ON threads(config_id, created_at DESC);
...
CREATE OR REPLACE TRIGGER trg_plugins_updated_at
    BEFORE UPDATE ON plugins FOR EACH ROW EXECUTE FUNCTION update_updated_at();
CREATE OR REPLACE TRIGGER trg_configs_updated_at
    BEFORE UPDATE ON configs FOR EACH ROW EXECUTE FUNCTION update_updated_at();
CREATE OR REPLACE TRIGGER trg_threads_updated_at
    BEFORE UPDATE ON threads FOR EACH ROW EXECUTE FUNCTION update_updated_at();
CREATE OR REPLACE TRIGGER trg_replies_updated_at
    BEFORE UPDATE ON replies FOR EACH ROW EXECUTE FUNCTION update_updated_at();